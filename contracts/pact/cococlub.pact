(module cococlub-nfts 'coco-club
  "Coconut Club NFTs"

    (defconst COCOS_CREATED_COUNT_KEY "cocos-count-key")
    (defconst COCOS_MAX_KEY "cocos-max-key")
    (defconst COCOS_URI_KEY "cocos-uri-key")
    (defconst COCOS_STATE_KEY "cocos-state")
    (defconst MINT_CHAIN_ID_KEY "mint-chain-id-key")
    (defconst PRICE_KEY "price-key")
    (defconst MARKET_FEE_KEY "market-fee")
    (defconst ADMIN_KEYSET (read-keyset 'coco-club ))
    (defconst ADMIN_ADDRESS "k:f7278eeaa55a4b52c281fa694035f82a43a6711eb547fc1ab900be1ccf9fb409") ; Change this address
    ...
    (defcap OWNER (id:string)
        @doc "Enforces that an account owns a cococlub nft"
        (let ((nft-owner (at "owner" (read nfts id ["owner"]))))
    ...
    (defcap PUT_ON_SALE (id:string owner:string price:decimal)
      @doc "Emitted event when a cococlub is put on sale "
      @event true
    )

    (defcap REMOVED_FROM_SALE (id:string owner:string)
      @doc "Emitted event when a cococlub is removed from sale "
      @event true
    )
    ...
    (defun initialize ()
        ...
        (insert counts COCOS_CREATED_COUNT_KEY {"count": 0})
        (insert counts COCOS_MAX_KEY {"count": 1111})
        (insert values MINT_CHAIN_ID_KEY {"value": "2"})
        (insert values COCOS_URI_KEY {"value": "cococlubnft.com/"})
        (insert values COCOS_STATE_KEY {"value": "0"}) ; 0 -> Seed, 1 -> Sapling, 2 -> Tree!
        (insert decimals PRICE_KEY {"value": 1.0})
        (insert decimals MARKET_FEE_KEY {"value": 0.10})
    )
    ...
    (defschema decimals-schema
        @doc "Basic schema used for decimal things"
        value:decimal
    )
    ...
    (deftable decimals:{decimals-schema})
    ...
    (defun put-id-for-sale (id:string price:decimal)
        @doc "Puts a cococlub up for sale"
        (with-capability (OWNER id)
            (enforce (> price 0.0) "Price must be positive")
            (let 
                (
                    (owner (at "owner" (get-cococlub-fields-for-id ["owner"] id )))
                )
    ...
    (defun remove-id-from-sale (id:string)
        @doc "Removes a cococlub up from selling"
        (with-capability (OWNER id)
            (let 
                (
                    (owner (at "owner" (get-cococlub-fields-for-id ["owner"] id )))
    ...
    (defun buy-id-on-sale (id:string curr-user:string)
        @doc "Buys a cococlub that was put up for sale"
        (with-capability (ACCOUNT_GUARD curr-user)
            (enforce-cococlub-on-sale id)
            (let* 
                (
                    (cococlub-data (get-cococlub-fields-for-id [] id ))
                    (original-owner (at "owner" cococlub-data))
                ...
                (write marketplace id {
                    "id": id,
                    "for-sale": false, 
                    "price": -1.0,  ; Invalid price so cococlub can't be sold
                    "updated-at": (at "block-time" (chain-data)),
                    "owner": curr-user
                })
                (update nfts id (+ {"owner": curr-user} cococlub-data ))
                (emit-event (BOUGHT id curr-user original-owner price))
            )
        )
    )

    (defun mint-cococlub (id:string)
        @doc "Mints a new cococlub"
        (require-capability (PRIVATE))
        (let 
            (
                (mint-chain-id (get-value MINT_CHAIN_ID_KEY))
            )
            (enforce (= (curr-chain-id) mint-chain-id) "Can only mint on specific chain")
        )
        (let ((id (id-for-new-cococlub)))
            (insert nfts id data)
        )
        (increase-count COCOS_CREATED_COUNT_KEY)
    )
    ...
    (defun set-price(price-value:decimal)
        @doc "Set the price"
        (with-capability (ADMIN)
            (update decimals PRICE_KEY {"value": price-value})
        )
    )
    ...
    (defun transfer:string
        ...
        (enforce (= 1.0 amount) "Only 1 cococlub can be transferred")
    ...
    ;;;;;; COCOCLUB NON STATE MODIFYING HELPER FUNCTIONS ;;;;;;;;;

    (defun get-decimal(key:string)
        @doc "Gets decimal for key"
        (at "value" (read price PRICE_KEY ["value"]))
    )
    ...
    (defun enforce-cococlub-on-sale (id:string)
        @doc "Enforces the cococlub for the ID is on sale"
        (let
            (
                ; Get the owner for the cococlub
                (current-owner (at "owner" (get-cococlub-fields-for-id ["owner"] id)))
    ...
    (defun cococlubs-owned-by (owner:string)
        @doc "All kitties owned by someone"
        (select nfts ["id"] (where "owner" (= owner)))
    )

    (defun get-market-fee-from-price (price:decimal)
        @doc "Market fee cost for cococlub sold at a given price"
        (* price (get-decimal MARKET_FEE_KEY))
    )

    (defun get-to-seller-amount-from-price (price:decimal)
        @doc "Amount that goes to a seller when cococlub sold at a given price"
        (* price (- 1 (get-decimal MARKET_FEE_KEY)))
    )

    (defun get-cococlub-fields-for-ids (fields:list ids:list) 
        @doc "Return fields for a list of ids"
        (map 
            (get-cococlub-fields-for-id fields)
            ids
        )
    )

    (defun get-cococlub-fields-for-id (fields:list id:string )
    ...
    (defun uri:string (id:string)
        @doc "Give URI for ID. If not supported, return \"\" (empty string)."
        (+ (+ (get-value COCOS_URI_KEY) id) (+ "_" (get-value COCOS_STATE_KEY))
    )

    (defun id-for-new-cococlub ()
        @doc "Returns an id for a new cococlub to be minted"
        (+ (+ (curr-chain-id) ":") (int-to-str 10 (get-count COCOS_CREATED_COUNT_KEY)))
    )
    ...
    (defun get-all-on-sale ()
        ...
        (select marketplace ["id"] (where "for-sale" (= true)))
    )
